...
- [x] 6. Phase 1: グループ A（コア機能）+ 3D エンジン基礎
  ...
  - サブタスク進捗（`tasks/group-a.md` と同期）:
    ...
    - [ ] A5. 属性パネル
    - [ ] A6. プロジェクト一覧
    - [x] A7. ページルーティングとレイアウト統合
    - [x] A8. Phase 1 結合テスト

- [x] 7. Phase 1 チェックポイント
  - グループ A の全エンドポイントが `_stub: false` を返すこと、全テストが通ることを確認
  - ビルド: 全パッケージ成功、テスト: 30テスト全パス（backend 27 + shared 3）

- [x] 8. Phase 2: グループ B（BIM シミュレーション）
  - 詳細は `tasks/group-b.md` を参照
  - 数量算出、コスト概算、干渉チェック
  - _要件: 8, 9, 10_
  - サブタスク進捗（`tasks/group-b.md` と同期）: B0〜B6 完了（B1.4 / B2.3 / B3.3 のプロパティテストはオプション）

- [x] 9. Phase 2: グループ C（工程管理）
  - 詳細は `tasks/group-c.md` を参照
  - 工程情報、ガントチャート、クリティカルパス
  - _要件: 4, 5, 6_

- [x] 10. Phase 2 チェックポイント
  - グループ A, B, C の全エンドポイントが `_stub: false` を返すこと、全テストが通ることを確認
  - ビルド: 全パッケージ成功、テスト: 51テスト全パス（backend 33 + shared 18）

- [x] 11. Phase 3: ブラッシュアップ + 品質向上
  - [x] 11.1 パフォーマンス最適化
    - 100 ブロック以上での数量算出、干渉チェック、ガントチャートのパフォーマンステスト
    - ボトルネックの特定と最適化
    - _要件: 8.1, 10.1, 5.1_
  - [x] 11.2 エッジケース・エラーハンドリング強化
    - 無効入力、空プロジェクト、ブロック不足、循環依存等のエラーケース網羅テスト
    - _要件: 3.4, 4.4, 4.5, 10.4, 5.5, 6.5_
  - [x] 11.3 4D シミュレーション（工程アニメーション）の実装
  ...
  - [x] 11.4 フロントエンド UX 磨き込み
    - スナップ機能の精度向上、操作ハンドルの改善、レスポンシブ対応
    - _要件: 2.3, 2.4_
  - [x] 11.6 スナップ UX 改善（ユーザーフィードバック）
    - スナップ距離を 10mm → 100mm 以上に拡大し、大雑把に置いても揃う体験にする
    - ドラッグ中にリアルタイムでスナップガイドを表示する（現状はドロップ後のみ）
    - スナップ発動時のフィードバックを強化（色変更、トースト通知、吸着アニメーション）
    - ガイドラインの表示時間を延長 or スナップ中は常時表示
    - _要件: 2.3, 2.4_
  - [x] 11.7 属性パネルに位置・回転の数値入力（ユーザーフィードバック）
    - 選択ブロックの position（x, y, z）と rotation（x, y, z）を属性パネルに数値フィールドとして表示
    - 数値を直接編集すると 3D キャンバス上のブロック位置がリアルタイムに反映される
    - ざっくりドラッグで配置 → 数値で微調整、のワークフローを実現
    - _要件: 3.1, 3.2_
  - [ ]* 11.5 プロパティベーステストの拡充
    - 全 Property のイテレーション数を 100 → 500 に増加
    - _要件: 全要件共通_

- [x] 12. Phase 3 チェックポイント
  - 100 ブロックでの操作がストレスなく動作すること、全エラーケースで適切なメッセージが返ることを確認
  - ビルド: 全パッケージ成功（チャンク分割対応済み）、テスト: 100テスト全パス（backend 56 + shared 44）

- [x] 13. Phase 4: グループ D（AI 活用）
  - 詳細は `tasks/group-d.md` を参照
  - AI 構造提案、自然言語モデリング、画像認識
  - _要件: 15, 16, 17_

- [x] 14. Phase 4: グループ E（建設チェック）
  - 詳細は `tasks/group-e.md` を参照
  - 構造チェック、法規チェック、環境シミュレーション、安全管理
  - _要件: 18, 19, 20, 21_

- [x] 15. Phase 4: グループ F（データ連携）
  - 詳細は `tasks/group-f.md` を参照
  - ダッシュボード、IFC 入出力、プロジェクト比較
  - _要件: 22, 11, 12, 23_

- [x] 16. Phase 4 チェックポイント（最終）
  - 全グループ（A〜F）の全エンドポイントが `_stub: false` を返すこと、全テストが通ること、Agent API が OpenAPI スキーマと整合することを確認
  - ビルド: 全パッケージ成功、テスト: 100テスト全パス
  - Mock モードでのベース開発完了
